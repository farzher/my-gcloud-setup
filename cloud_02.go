...
if [ -d /srv/web/repo/.git ] && git -C /srv/web/repo remote get-url origin >/dev/null 2>&1; then echo READY_GITHUB; fi
if [ -x /srv/web/repo/ops/deploy.sh ] && [ -s /srv/web/repo/.hermes.md ] && [ -x /usr/local/bin/backup-web ] && systemctl is-enabled --quiet web-backup.timer && systemctl is-active --quiet web-backup.timer && systemctl is-active --quiet nginx && systemctl is-active --quiet postgresql && [ -s /srv/web/current-slot ]; then echo READY_WEB; fi
`
	if domain == "" {
