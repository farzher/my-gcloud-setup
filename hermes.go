...
- Whenever creating a site or materially changing its framework, routes, static paths, build output, or response semantics, review ops/nginx.conf in the same change and update it when a faster safe delivery/cache policy is available. Do not wait for a separate performance request.
...
- Prefer Nginx direct serving for static/build assets when it is straightforward and correct. For a static-only site, keep Node out of the normal request path and serve the site directly from Nginx while preserving /healthz. Give content-hashed or fingerprinted immutable assets long public immutable caching; use sensible shorter caching or revalidation for mutable/non-hashed assets.
